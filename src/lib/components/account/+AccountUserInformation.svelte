<script lang="ts">
	...
	let userInformationOpen: boolean = false;
	let shippingInformationOpen: boolean = false;
...
<form on:submit={handleSubmit} class="account-user-information">
	<Accordion bind:open={userInformationOpen}>
		<div class="--accordion-header" slot="head">
			<h4>{trans("page.account.userInformation.label")}</h4>

			{#if userInformationOpen}
			...
			<div class="--group-2">
				...
				<div class="input-group">
					...
					<label class="floating-label" for="firstName"
						>{trans("page.account.firstName.label")}</label
					>
				</div>
	...
	<Accordion bind:open={shippingInformationOpen}>
		<div class="--accordion-header" slot="head">
			<h4>{trans("page.account.shippingInformation.label")}</h4>

			{#if shippingInformationOpen}
				<CaretUpIcon fill="fill-gray-500" />
			{:else}
				<CaretDownIcon fill="fill-gray-500" />
			{/if}
